...
wandb.init(
    ...
    config={
        ...
        "design_algorithm": advanced_settings["design_algorithm"],
        "max_trajectories": advanced_settings["max_trajectories"],
        "acceptance_rate": advanced_settings["acceptance_rate"],
    }
)
...
wandb.config.update({
    ...
    "binder_name": target_settings["binder_name"],
    "use_multimer_design": advanced_settings["use_multimer_design"],
})


### start design loop
while continue_designing:
    ...
    if not any(os.path.exists(os.path.join(design_paths[trajectory_dir], design_name + ".pdb")) for trajectory_dir in trajectory_dirs):
